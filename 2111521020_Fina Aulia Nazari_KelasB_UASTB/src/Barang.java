...
public class Barang implements Penitipan{
    ...
    //koneksi database
    Connection conn;
    String url = "jdbc:mysql://localhost:3306/penitipan_barang";

    int jamm, jamk, waktu;

    Scanner input = new Scanner(System.in);
    int  tagihan, bayar, kembalian, jnsBarang;
    String namaPemilik; 
    int noPenitipan;
    ...
    public void jenisBarang() {
        ...
        System.out.print("Masukkan Jenis Barang (angka)\t: ");
        this.jnsBarang = input.nextInt();
        input.nextLine();

        //perulangan while
        while(jnsBarang<1 || jnsBarang>2){
    ...
    public void insertData() throws SQLException{
        //insert database
        String sql = "INSERT INTO barang (no_penitipan,jenis_barang,nama_pemilik,jam_masuk) VALUES ('"+this.noPenitipan+"', '"+this.jnsBarang+"','"+this.namaPemilik+"','"+this.jamm+"' )";
    ...
    public void showData() throws SQLException{
        int count = 1;
        String sql = "SELECT no_penitipan, nama_pemilik, jenis_barang, jam_masuk FROM barang";
        conn = DriverManager.getConnection(url,"root","");
        Statement statement = conn.createStatement();
        ResultSet result = statement.executeQuery(sql);
        ...
        while(result.next()){
            ...
            System.out.println(result.getInt("jenis_barang"));
    ...
    public void editData()throws SQLException {
        try{
            ...
            //mengakses database
            String sql = "SELECT * FROM barang WHERE no_penitipan='"+edit+"'";
            conn = DriverManager.getConnection(url,"root","");
            Statement statement = conn.createStatement();
            ResultSet result = statement.executeQuery(sql);
                ...
                //percabangan switch case
                    switch (pil){
                        case 1:
                            ...
                            //update database
                            sql = "UPDATE barang SET no_penitipan = '"+editPenitipan+"' WHERE no_penitpan ='"+edit+"'";
                        ...
                        case 2:
                            ...
                            //update database
                            sql = "UPDATE barang SET nama_pemilik = '"+editNama+"' WHERE no_penitipan ='"+edit+"'";
                            if(statement.executeUpdate(sql) > 0){
                        ...
                        case 3:
                            System.out.print("Jenis barang <"+result.getInt("jenis_barang")+">\t: ");
                            int editJenis = input.nextInt();
                            //update database
                            sql = "UPDATE barang SET jenis_barang = '"+editJenis+"' WHERE no_penitipan ='"+edit+"'";
                            if(statement.executeUpdate(sql) > 0){
                                System.out.println("\n```Berhasil mengubah Jenis Barang menjadi |"+editJenis+"|```\n");
                            }
                            break;
    ...
    public void deleteData()throws SQLException{
        try{
            ...
            //menghapus database
            String sql = "DELETE FROM barang WHERE no_penitipan LIKE'%"+del+"%'";
            conn = DriverManager.getConnection(url,"root","");
            Statement statement = conn.createStatement();
            if(statement.executeUpdate(sql) > 0){
